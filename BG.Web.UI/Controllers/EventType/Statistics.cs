...
namespace BG.Web.UI.Controllers
{
	...
	public partial class EventTypeController
	{
		...
		/// </summary>
		[HttpGet]
		[Route("statistics", Name = "EventTypeStatisticsGet")]
		public ActionResult Statistics()
		{
			var model = new EventTypeStatistics();

			var eventtype = _eventtypeRepository.GetAll();

			// Count
			model.Count = eventtype.Count();

			// Latest
			model.Latest = eventtype.OrderByDescending(c => c.Id).Take(3).Select(i => new
			{
				name = i.Name,
				...
				navigationPath = Url.Action(nameof(EventTypeController.Edit), nameof(EventTypeController).RemoveControllerSuffix(), new { @id = i.Id })
			}).Cast<object>().ToList();
            ...
            eventtype = eventtype.Where(c => c.UTCCreation.Date >= minDate.Date);
            eventtype = eventtype.Where(c => c.UTCCreation.Date <= maxDate.Date);
			...
			for (DateTime date = minDate.Date; date <= maxDate.Date; date = date.AddDays(1))
			{
				...
				var dayValue = eventtype.Where(c => c.UTCCreation.Date.Equals(date)).Count();
