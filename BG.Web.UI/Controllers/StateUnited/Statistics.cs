...
namespace BG.Web.UI.Controllers
{
	...
	public partial class StateUnitedController
	{
		...
		/// </summary>
		[HttpGet]
		[Route("statistics", Name = "StateUnitedStatisticsGet")]
		public ActionResult Statistics()
		{
			var model = new StateUnitedStatistics();

			var entity = _stateunitedRepository.GetAll();
			...
			model.Latest = entity.OrderByDescending(c => c.Id).Take(3).Select(i => new
			{
				...
				// navigationPath = Url.Action(nameof(StateUnitedController.Edit), nameof(StateUnitedController).RemoveControllerSuffix(), new { @id = i.Id })
			}).Cast<object>().ToList();
