...
namespace BG.Web.UI.Controllers
{
	...
	public partial class EventTrackingParameterController
	{
		...
		[HttpGet]
		[Route("statistics", Name = "EventTrackingParameterStatisticsGet")]
		public ActionResult Statistics()
		{
			var model = new EventTrackingParameterStatistics();

			var entity = _eventtrackingparameterRepository.GetAll();
			...
			model.Latest = entity.OrderByDescending(c => c.Id).Take(3).Select(i => new
			{
				...
				// navigationPath = Url.Action(nameof(EventTrackingParameterController.Edit), nameof(EventTrackingParameterController).RemoveControllerSuffix(), new { @id = i.Id })
			}).Cast<object>().ToList();
