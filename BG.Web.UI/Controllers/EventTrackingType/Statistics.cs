...
namespace BG.Web.UI.Controllers
{
	...
	public partial class EventTrackingTypeController
	{
		...
		[HttpGet]
		[Route("statistics", Name = "EventTrackingTypeStatisticsGet")]
		public ActionResult Statistics()
		{
			var model = new EventTrackingTypeStatistics();

			var entity = _eventtrackingtypeRepository.GetAll();
			...
			model.Latest = entity.OrderByDescending(c => c.Id).Take(3).Select(i => new
			{
				...
				// navigationPath = Url.Action(nameof(EventTrackingTypeController.Edit), nameof(EventTrackingTypeController).RemoveControllerSuffix(), new { @id = i.Id })
			}).Cast<object>().ToList();
