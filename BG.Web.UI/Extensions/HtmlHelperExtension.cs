...
namespace BG.Web.UI.Extensions
{
    ...
    public static partial class HtmlHelperExtensions
    {
            ...
            string fieldName, string actionName, string sortField, QueryOptions queryOptions)
        {
            ...
            return new MvcHtmlString(string.Format("<a href=\"{0}\">{1} {2}</a>",
                ...
                {
                    SortField = sortField,
                    SortOrder = (isCurrentSortField && queryOptions.SortOrder.ToString() == SortOrder.ASC.ToString())
                        ? SortOrder.DESC : SortOrder.ASC,
                    LotId = queryOptions.LotId,
        ...
        private static string BuildSortIcon(bool isCurrentSortField, QueryOptions queryOptions)
        {
            ...
            if (isCurrentSortField)
            {
                ...
                if (queryOptions.SortOrder.ToString() == SortOrder.DESC.ToString())
                    sortIcon += "-alt";
            }
        ...
        UrlHelper urlHelper, QueryOptions queryOptions, string actionName)
        {
            ...
            "<a href=\"{0}\"><span aria-hidden=\"true\">&larr;</span> Предыдущая</a>",
    ...
    UrlHelper urlHelper, QueryOptions queryOptions, string actionName)
        {
            return string.Format(
            "<a href=\"{0}\">Следующая <span aria-hidden=\"true\">&rarr;</span></a>",
            urlHelper.Action(actionName, new
            {
                SortOrder = queryOptions.SortOrder,
                SortField = queryOptions.SortField,
        ...
        public static MvcHtmlString BuildKnockoutDepartmentsLink(this HtmlHelper htmlHelper, string DepartmentTitle)
        {
            var urlHelper = new UrlHelper(htmlHelper.ViewContext.RequestContext);

            return new MvcHtmlString(string.Format(
                "<a href=\"#\" data-bind=\"click: pagingService.departmentFilter\"" +

                " deptitle = \"{0}\"" + " >" + DepartmentTitle + "</a>",
               DepartmentTitle));
        }
    ...
    "           <a href=\"{0}\" data-bind=\"click: pagingService.previousPage\">Предыдущая</a></li>" +
    "        <li data-bind=\"css: pagingService.buildNextClass()\">" +
    "           <a href=\"{0}\" data-bind=\"click: pagingService.nextPage\">Следущая</a></li></li>" +
    "    </ul>" +
    "</nav>",
            @urlHelper.Action(actionName)
            ));
